...
<body>
    <nav class="nav">
        ...
        <?php 
            $sql = mysqli_query($conn_db, 'SELECT pageID, pageTitle FROM pages');
    ...
    <main class="main">
        <?php
            ...
            if(!isset($_GET['p'])){
                $q = mysqli_query($conn_db, "SELECT * FROM pages WHERE pageID='1'");
            } else { //load requested page based on the id
                $id = $_GET['p']; //get the requested id
                $q = mysqli_query($conn_db, "SELECT * FROM pages WHERE pageID='$id'");
            }
